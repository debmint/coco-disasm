...
void PrintLine(char *pfmt,struct printbuf *pb)
{
	...
	if( InProg && (nl=FindLbl(SymLst[(int)strpos(lblorder,'L')],CmdEnt)) ) {
...
void UpString(char *s)	/* Translate a string to uppercase */
{
...
void StartPage()
{
	...
	printf("OS9 Cross Disassembler - Ver. 01.00.00    %02d/%02d/%02d %02d:%02d:%02d      Page %03d\n\n",
			tm->tm_mon + 1, tm->tm_mday, tm->tm_year + 1900,
			tm->tm_hour, tm->tm_min, tm->tm_sec,
			PgNum);
	PgLin=2;

	if( PgNum == 1 ) {	/* print disassembler info on first page */
		LinNum=1;
...
void BlankLine()	/* Prints a blank line */
{
	if( !PgLin || PgLin>(PgDepth-6))
		StartPage();
...
void OS9Modline()
{
	...
	sprintf(prtbf->operand,"%s,%s,$%02x,$%02x,%s",
			...
			(mhd->M_Type),(mhd->M_Revs),
			FindLbl(LL,ModExe)->sname);
	if( HdrLen == 13 ) {
		strcat(prtbf->operand,",");
		/*strcat(prtbf->operand,FindLbl(SymLst[strpos(lblorder,'D')],*/
		strcat(prtbf->operand,FindLbl(SymLst[0],
					ModData)->sname);
	}
...
/*	PrintLine("%5d  %06x %-12s%s %-10s %-6s %s\n",prtbf);*/
	...
	if( (nl=FindLbl(SymLst[strpos(lblorder,'L')],Pc)) ) {
		...
		strcpy(prtbf->mnem,"equ");strcpy(prtbf->operand,"*");
...
void OS9DataPrint()
{
	...
	/*if( (dta=ListRoot('D')) ) {*/
	if( (dta=SymLst[0]) ) {	/* special tree for OS9 data defs */
		BlankLine();
		printf("%5d %22s%s\n",LinNum++,"",what);++PgLin;
		if(WrtSrc)
			fprintf(outpath,"%s\n",what);
		...
		if( (srch->myaddr) ) { /* i.e., if not D000 */
			strcpy(pbf->mnem,"rmb");sprintf(pbf->operand,"%d",
					srch->myaddr);
			CmdEnt=PrevEnt=0;
...
void ListData(struct nlist *me,int upadr)
{
	...
	CmdEnt=me->myaddr;PrevEnt=CmdEnt;
	PrintLine(realcmd,pbf);
...
void TellLabels(struct nlist *me, int flg )
{
	...
	if( (flg<0) || (flg == me->stdnam) ) {
		if(!HadWrote) {
			BlankLine();
			printf(ClsHd,LinNum++,"",NowClass);++PgLin;
			if(outpath)
		...
		strcpy(pb->lbnm,me->sname); strcpy(pb->mnem,"equ");
